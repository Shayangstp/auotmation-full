...
export const getProInfo = (storeAsCode, coCode) => {
    return http.get(`${config.localapi}/material/selectedGood/` + storeAsCode + '/' + coCode,{timeout: 30000})
}
...
export const getReqProcess = (processValues) => {
    return http.get(`${config.localapi}/global/process`, {params:processValues},{timeout: 30000})
}
...
export const getCoDepartments = (companyCode, location) => {
    return http.get(`${config.localapi}/dep/findDeps/` + companyCode + '/' + location,{timeout: 30000})
}
// get company users with office
export const getAllUsersWithOffice = (companyCode, location) => {
    return http.get(`${config.localapi}/user/wrote/` + companyCode + '/' + location + '/' + localStorage.getItem('id'),{timeout: 30000})
}
// get all users
...
export const getCoUsers = (info, companyCode, location) => {
    return http.get(`${config.localapi}/user/findInCompany/` + info + '/' + companyCode + '/' + location, { params: { id: localStorage.getItem('id') } },{timeout: 30000})
}
// get all users in dep
export const getDepartmentUsers = (depName, company, location) => {
    let dep = { code: '', name: depName }
    return http.get(`${config.localapi}/user/toList/` + company + '/' + location, { params: dep },{timeout: 30000})
}
...
export const getMenu = (pMenuId) => {
    return http.get(`${config.localapi}/menu/title`,{timeout: 30000})
}

// check permission
export const checkPermission = () => {
    return http.get(`${config.localapi}/access/present/pers/` + localStorage.getItem('id'),{timeout: 30000})
}

// permissions updated
export const gotPermissions = () => {
    return http.patch(`${config.localapi}/access/changedPer/` + localStorage.getItem('id'),{timeout: 30000})
}

// get not seen reqs
export const getNotSeenReqs = (type) => {
    return http.get(`${config.localapi}/action/seen/` + localStorage.getItem('id') + '/' + type,{timeout: 30000})
}
...
export const getLastNewReqs = () => {
    return http.get(`${config.localapi}/action/bell/`, { params: { id: localStorage.getItem('id') } },{timeout: 30000})
}

//get units
export const getUnits = () => {
    return http.get(`${config.localapi}/inventory/material/units`,{timeout: 30000})
}
...
export const getCitiesByType = (type) => {
    return http.get(`${config.localapi}/city/accordigToList`, { params: { type: type } },{timeout: 30000})
}
...
export const getCurrentReqItems = (reqId, reqType) => {
    return http.get(`${config.localapi}/global/items/` + reqId + '/' + reqType,{timeout: 30000})
}
// Get Current Req History
export const getCurrentReqHistory = (commentsStatus, reqId, type) => {
    return http.get(`${config.localapi}/action/findComments/` + commentsStatus + '/' + reqId + '/' + type,{timeout: 30000})
}

export const getRequestsList = (filterValues) => {
    return http.get(`${config.localapi}/action/reqList`, { params: filterValues },{timeout: 30000})
}
//let res = JSON.stringify(query)

export const checkReqUpdateDate = (requestId, actionId, type) => {
    return http.get(`${config.localapi}/action/checkDate/` + requestId + `/` + actionId + '/' + type,{timeout: 30000})
}

export const checkItemUpdateDate = (itemId, actionId) => {
    return http.get(`${config.localapi}/global/checkDateOfItem/` + itemId + `/` + actionId,{timout: 30000})
}

// Get to persons
export const getToPersonByRole = (role, location, company, existRole, dep, task) => {
    return http.get(`${config.localapi}/user/findPerson/inGroup`, { params: { roles: role, location: location, company: company, exist: existRole, department: dep, task: task } },{timeout: 30000})
}
...
export const sendSmsCode = (code, value, status, userId) => {
    const id = localStorage.getItem('id') !== null ? localStorage.getItem('id') : userId;
    return http.get(`${config.localapi}/user/confirmSMS/` + id + '/' + code, { params: { justdelete: value, valueCode: status } },{timeout: 30000});
}

export const getMyReqsList = (filterValues) => {
    return http.get(`${config.localapi}/global/myRequests`, {params: filterValues},{timeout: 30000});
}

export const getNumberOfStaff = (minmaxValues) => {
    return http.get(`${config.localapi}/chart/numberOfPersonel`, { params: minmaxValues },{timeout: 30000});
}
