...
import { RsetFormErrors, RsetCompanyNames, handleReqsList, RsetLoading, RsetUsersByRole } from "./mainSlices";
import { postAction, getCoUsers, getToPersonByRole } from "../../Services/rootServices";
import { postWarehouseReq, postWarehouseReqItems, getWarehouseReqItems, editWarehouseItem, postDeliveryForReqItem, sendToWarehousePermission } from "../../Services/warehouseReqService";
import { RsetCurrentReqItems, RsetCurrentReqInfo, RsetCurrentReqToPerson, RsetCurrentReqItem } from "./currentReqSlice";
...
export const handleAddNewWarehouseReqItem = createAsyncThunk(
    ...
    async (obj, { dispatch, getState }) => {
        const { warehouseReqItemName, warehouseReqItemAmount, warehouseReqItemUnit, warehouseReqItemMainPlace,
            warehouseReqItemSubPlace, warehouseReqItemDescription
        } = getState().warehouse;
        ...
        const result = items.reduce((acc, item) => {
            const itemInAcc = acc.find(itemAcc => itemAcc.itemName === item.itemName);
    ...
    async (reqType, { dispatch, getState }) => {
        ...
        try {
            ...
            if (sendOptions === '2' && usersByRole === '') {
                ...
                const { currentReqItems } = getState().currentReq;
                const { warehouseReqMyRequest, warehouseReqApplicantFName, warehouseReqApplicantLName, warehouseReqSupervisor, warehouseReqDescription,
                    warehouseReqIsProject, warehouseReqProjectCode
                } = getState().warehouse;
                if (currentReqItems.length !== 0) {
                    const warehouseReqValues = {
                        applicantName: warehouseReqMyRequest ? undefined : warehouseReqApplicantFName,
                        applicantLname: warehouseReqMyRequest ? undefined : warehouseReqApplicantLName,
                        // project 1 , not 0
                        forProject: warehouseReqIsProject ? 1 : 0,
                        projectCode: warehouseReqProjectCode !== '' ? warehouseReqProjectCode : undefined,
                        description: warehouseReqDescription !== '' ? warehouseReqDescription : undefined,
                    }
                    const warehouseReqValuesRes = await postWarehouseReq(warehouseReqValues);
                    if (warehouseReqValuesRes.data.code === 415) {
                        ...
                        currentReqItems.map(async (value, index) => {
                            const ItemsValue = {
                                ...
                                itemUnit: value.itemUnit.value !== undefined ? value.itemUnit.value : '',
                                mainPlace: value.itemMainPlace,
                                subPlace: value.itemSubPlace === '' ? value.itemMainPlace : value.itemSubPlace,
                                comment: value.itemDescription !== '' ? value.itemDescription : undefined,
                                ...
                                if (currentReqItems.length === warehouseSavedItems) {
                                    ...
                                    if (sendOptions === '1') {
                                        ...
                                        if (postActionRes.data.code === 415) {
                                            ...
                                            successMessage('درخواست با موفقیت ثبت و در کارتابل سرپرست قرار گرفت!!');
                                        }
                                        ...
                                        if (postActionRes.data.code === 415) {
                                            dispatch(RsetLoading(false));
                                            dispatch(handleResetWarehouseReq());
                                            successMessage('درخواست با موفقیت ثبت و در کارتابل شخص انتخاب شده قرار گرفت!!');
                                        }
                                        ...
                                        const postActionRes = await postAction(actionValues);
                                        if (postActionRes.data.code === 415) {
                                            const persons = await getToPersonByRole('3, 36', user.Location, user.CompanyCode, 1, null, '0');
                                            ...
                                            if (toPersons !== '') {
                                                ...
                                                if (postActionRes.data.code === 415) {
                                                    dispatch(RsetLoading(false));
                                                    dispatch(handleResetWarehouseReq());
                                                    successMessage('درخواست با موفقیت ثبت و در کارتابل انبار قرار گرفت!!');
                                                }else{
                ...
                const sendEditedData = async () => {
                    currentReqItems.map(async (value, index) => {
                        if (value.itemName === "") {
                            ...
                            const updateReqValues = {
                                ...
                                itemName: value.itemName,
                                itemAmount: value.itemAmount,
                                itemUnit: value.itemUnit.value !== undefined ? value.itemUnit.value : value.itemUnitCode,
                                mainPlace: value.itemMainPlace,
                                subPlace: value.itemSubPlace === null ? value.itemMainPlace : value.itemSubPlace,
                                comment: value.itemDescription !== null ? value.itemDescription : undefined,
                                action: 3,
                                userId: localStorage.getItem('id')
                            }
                            const itemUpdateRes = await editWarehouseItem(value.itemId, updateReqValues);
                        }
                    })
                    if (warehouseDeletedReqItems.length !== 0) {
                        warehouseDeletedReqItems.map(async itemId => {
                            ...
                            const deletedReqItemRes = await editWarehouseItem(itemId, updateReqValues);
                        })
                    }
                    currentReqItems.map(async (value, index) => {
...
export const selectWarehouseReqMyRequest = (state) => state.warehouse.warehouseReqMyRequest;
export const selectWarehouseReqApplicantFName = (state) => state.warehouse.warehouseReqApplicantFName;
export const selectWarehouseReqApplicantLName = (state) => state.warehouse.warehouseReqApplicantLName;
export const selectWarehouseReqSupervisor = (state) => state.warehouse.warehouseReqSupervisor;
export const selectWarehouseReqIsProject = (state) => state.warehouse.warehouseReqIsProject;
export const selectWarehouseReqProjectCode = (state) => state.warehouse.warehouseReqProjectCode;
export const selectWarehouseReqItemName = (state) => state.warehouse.warehouseReqItemName;
export const selectWarehouseReqItemAmount = (state) => state.warehouse.warehouseReqItemAmount;
export const selectWarehouseReqItemUnit = (state) => state.warehouse.warehouseReqItemUnit;
export const selectWarehouseReqItemMainPlace = (state) => state.warehouse.warehouseReqItemMainPlace;
export const selectWarehouseReqItemSubPlace = (state) => state.warehouse.warehouseReqItemSubPlace;
export const selectWarehouseReqItemRattletrap = (state) => state.warehouse.warehouseReqItemRattletrap;
export const selectWarehouseReqItemConsumable = (state) => state.warehouse.warehouseReqItemConsumable;
export const selectWarehouseReqItemBorrowed = (state) => state.warehouse.warehouseReqItemBorrowed;
export const selectWarehouseReqItemDescription = (state) => state.warehouse.warehouseReqItemDescription;
export const selectWarehouseReqDescription = (state) => state.warehouse.warehouseReqDescription;
export const selectWarehouseReqItems = (state) => state.warehouse.warehouseReqItems;
export const selectWarehouseReqItemId = (state) => state.warehouse.warehouseReqItemId;
export const selectWarehouseDeletedReqItems = (state) => state.warehouse.warehouseDeletedReqItems;
export const selectSendDirectlyToWarehouse = (state) => state.warehouse.sendDirectlyToWarehouse;
