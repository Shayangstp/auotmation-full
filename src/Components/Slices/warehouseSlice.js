...
import { postWarehouseReq, postWarehouseReqItems, getWarehouseReqItems, editWarehouseItem, postDeliveryForReqItem } from "../../Services/warehouseReqService";
...
export const handleSubmitNewWarehouseReq = createAsyncThunk(
    ...
    async (reqType, { dispatch, getState }) => {
        try {
            const { user, usersByRole } = getState().mainHome;
            if(usersByRole !== ''){
                const { currentReqItems } = getState().currentReq;
