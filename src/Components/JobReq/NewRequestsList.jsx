...
const NewRequestsList = ({setPageTitle, loading, setLoading}) => {
    ...
    const setReqVisited = (request) => {
        handleReqVisited(request.reqInfo._id, true);
    }
    ...
    const operation = (actionCode) => {
        ...
        if(actionCode.userInfo._id === localStorage.getItem('id') && actionCode.process[index].action_code === 0 ){
            return(
                <Button
                    title='ویرایش'
        ...
        }else if(actionCode.process[index].action_code === 15){
        ...
        if (requests.length !== 0) {
            for (var i = 0; i < requests.length; i++) {
                ...
                var tableItem = {
                    ...
                    reqDate: moment.utc(xssFilters.inHTMLData(requests[i].process[index].date), 'YYYY/MM/DD').locale('fa').format('jYYYY/jMM/jDD'),
                    // reqUser: (requests[i].userInfo.first_name + " " + requests[i].userInfo.last_name),
                    // reqCompany: xssFilters.inHTMLData(requests[i].reqInfo.reqFromCompanyName.co_name),
                    // reqDep: requests[i].userInfo.department,
                    reqStatus: xssFilters.inHTMLData(requests[i].status),
