...
const WarehouseReqPage = ({ setPageTitle }) => {
      ...
      <div
        ...
        onClick={() => {
          if (userStatus === "user") {
            ...
            dispatch(
              handleUserImage({ userId: request.lastAcceptorId, status: 1 })
            );
          }
        }}
      >
  ...
  const status = (request) => {
    return (
      <a
        className="text-dark text-decoration-none cursorPointer serialHover"
        title={"مشاهده روند درخواست " + request.serial}
        onClick={() => {
          dispatch(
            handleReqProcess({
  ...
  const operation = (request) => {
    if (
      ...
      request.lastToPersons === localStorage.getItem("id") ||
      (request.lastToPersons !== null &&
        request.lastToPersons
          .split(",")
          .some((person) => person === localStorage.getItem("id")) === true)
    ) {
      return (
        <div className="d-flex justify-content-between flex-wrap">
          <Button
  ...
  useEffect(() => {
    const filterValues = {
      ...
      toDate: "null",
      type: 2,
      all: warehouseReqFilterActions ? 0 : 1,
    };
    dispatch(handleReqsList(filterValues));
  }, [warehouseReqFilterActions]);

  useEffect(() => {
    var acceptWarehouseModal = document.getElementById("modalBlur");
    if (
      (reqItemDeliveryModal === true || reqItemReceivedDetailsModal === true) &&
      acceptWarehouseModal !== null
    ) {
      document.getElementById("modalBlur").classList.add("blur-5");
    } else if (
      (reqItemDeliveryModal === false ||
        reqItemReceivedDetailsModal === false) &&
      acceptWarehouseModal !== null
    ) {
