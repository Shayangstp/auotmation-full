...
import {
    selectUser, selectFormErrors, RsetFormErrors, handleUsersByRoles, selectUsersByRole, RsetUsersByRole, selectUsersByRoleOptions,
    RsetSendOptions, selectSendOptions, selectLoading
} from '../Slices/mainSlices';
import {
    selectWarehouseReqMyRequest, RsetWarehouseReqMyRequest, selectWarehouseReqApplicantFName, RsetWarehouseReqApplicantFName,
    selectWarehouseReqApplicantLName, RsetWarehouseReqApplicantLName, selectWarehouseReqSupervisor, RsetWarehouseReqSupervisor,
    selectWarehouseReqIsProject, RsetWarehouseReqIsProject, selectWarehouseReqProjectCode, RsetWarehouseReqProjectCode,
    selectWarehouseReqDescription, RsetWarehouseReqDescription, handleResetWarehouseReq,
    handleSubmitNewWarehouseReq, handleSendToWarehousePermission, selectSendDirectlyToWarehouse
} from '../Slices/warehouseSlice';
import { RsetCurrentReqItems, selectCurrentReqItems } from "../Slices/currentReqSlice";
import { selectResetFormModal, RsetResetFormModal, selectSetSupervisorModal, RsetSetSupervisorModal } from "../Slices/modalsSlice";
...
const WarehouseNewRequest = ({ setPageTitle, setLocklinks }) => {
    ...
    const warehouseReqApplicantFName = useSelector(selectWarehouseReqApplicantFName);
    const warehouseReqApplicantLName = useSelector(selectWarehouseReqApplicantLName);
    ...
    const addWarehouseNewReqItem = (e) => {
        ...
        if (warehouseReqMyRequest) {
            ...
            if (warehouseReqApplicantFName !== '' && warehouseReqApplicantLName !== '') {
                if (!warehouseReqIsProject) {
                    dispatch(handleSubmitNewWarehouseReq());
                } else {
                    if (warehouseReqProjectCode !== '') {
            ...
            <section className="px-4 py-3 shadow lightGray2-bg mb-5 borderRadius border border-white border-2">
                ...
                <div className="position-relative">
                    ...
                    <Fragment>
                                ...
                                <div className="d-flex justify-content-center">
                                    <div className='ms-md-4'>
                                        ...
                                        <Form.Check
                                            ...
                                            onChange={() => { dispatch(RsetWarehouseReqMyRequest(!warehouseReqMyRequest)) }}
                                        />
                                    </div>
                                </div>
                            </Col>
                                ...
                                <Form>
                                    <Row>
                                        <Form.Group as={Col} md='4' xl='3' className='mb-4'>
                                            <Form.Label className='mb-1 required-field'>نام :</Form.Label>
                                            {warehouseReqMyRequest === true ?
                                                <Form.Control type="text" value={xssFilters.inHTMLData(user.FirstName)} name="fName" disabled />
                                                :
                                                <Form.Control type="text" value={warehouseReqApplicantFName} name="applicantFName" onChange={(e) => { dispatch(RsetWarehouseReqApplicantFName(e.target.value)) }} id='applicantFName' />
                                            }
                                            {!warehouseReqApplicantFName && (
                                        ...
                                        <Form.Group as={Col} md='4' xl='3' className='mb-4'>
                                            <Form.Label className='mb-1 required-field'>نام خانوادگی :</Form.Label>
                                            {warehouseReqMyRequest === true ?
                                                <Form.Control type="text" value={xssFilters.inHTMLData(user.LastName)} name="lName" disabled />
                                                :
                                                <Form.Control type="text" value={warehouseReqApplicantLName} name="applicantLName" onChange={(e) => { dispatch(RsetWarehouseReqApplicantLName(e.target.value)) }} id='applicantLName' />
                                            }
                                            {!warehouseReqApplicantLName && (
                                                <p className="font12 text-danger mb-0 mt-1">
                                        ...
                                        <Form.Group as={Col} lg='4' xl='6' className='mb-4 d-flex align-items-center pt-4'>
                                            <Form.Label className='mb-1 ms-2'>درخواست پروژه ای:</Form.Label>
                                            <div className="d-flex align-items-center mx-4">
                                                <input type='radio' name='warehouseReqIsProject' value={warehouseReqIsProject} defaultChecked={warehouseReqIsProject} onChange={(e) => { dispatch(RsetWarehouseReqIsProject(!warehouseReqIsProject)) }} />
                                                <Form.Label className='ms-2 font12 mb-0'> هست </Form.Label>
                                            </div>
                                            <div className="d-flex align-items-center">
                                                <input type='radio' name='warehouseReqIsProject' value={!warehouseReqIsProject} defaultChecked={!warehouseReqIsProject} onChange={(e) => { dispatch(RsetWarehouseReqIsProject(!warehouseReqIsProject)) }} />
                                                <Form.Label className='ms-2 font12 mb-0'> نیست </Form.Label>
                                            </div>
                                        </Form.Group>
                                        {warehouseReqIsProject ? <Form.Group as={Col} md='3' xl='3' className='mb-4'>
                                            <Form.Label className='mb-1'>سریال پروژه :</Form.Label>
                                            <Form.Control type="text" value={warehouseReqProjectCode} name="projectCode" onChange={(e) => { dispatch(RsetWarehouseReqProjectCode(e.target.value)) }} id='projectCode' />
                                        ...
                                        <hr className='mb-5' />
                                        <WarehouseReqNewItem />
                                        {currentReqItems.length !== 0 ? <WarehouseReqItems reqItemsOperation={true} /> : null}
                                        <hr className='mt-5' />
                                        <Form.Group as={Col} md='12' className='mb-4'>
                                            <Form.Label className='mb-1'>توضیحات :</Form.Label>
                                            <Form.Control as="textarea" rows={4} maxLength={50} name='requestDescription' value={warehouseReqDescription} onChange={e => dispatch(RsetWarehouseReqDescription(e.target.value))} />
                                        </Form.Group>
                                        <Col md='12' className="py-5">
                                            <Row className="lightGray2-bg p-4 shadow borderRadius border border-white">
                                                <Col md='12'>
                                                ...
                                                <Form.Group as={Col} md='4' xl='3' className='mb-4'>
                                                    ...
                                                    {user.Supervisor === undefined ? null :
                                                        <InputGroup>
                                                            <InputGroup.Radio name='sendOptions' value='1' checked onChange={(e) => { dispatch(RsetSendOptions(e.target.value)) }} aria-label="جهت انتخاب کلیک کنید." />
                                                ...
                                                <Form.Group as={Col} md='4' xl='3' className='mb-4'>
                                                    <Form.Label className='mb-1 required-field'>شخص دیگر :</Form.Label>
                                                    <InputGroup className="radioInputGroup">
                                                        <InputGroup.Radio name='sendOptions' value='2' onChange={(e) => { dispatch(RsetSendOptions(e.target.value)) }} aria-label="جهت انتخاب کلیک کنید." />
                                                        <Select isSearchable value={usersByRole} name='usersByRole' options={usersByRoleOptions} onChange={(option) => { dispatch(RsetUsersByRole(option)) }} placeholder='ارسال به ...' className="w-100" />
                                                    </InputGroup>
                                                </Form.Group>
                                                {sendDirectlyToWarehouse === true ?
                                                    <Form.Group as={Col} md='4' xl='3' className='mb-4'>
                                                        <Form.Label className='mb-1 required-field'>انبار :</Form.Label>
                                                        <InputGroup className="radioInputGroup align-items-center">
                                                            <InputGroup.Radio name='sendOptions' value='3' onChange={(e) => { dispatch(RsetSendOptions(e.target.value)) }} aria-label="جهت انتخاب کلیک کنید." />
                                                            <span className="ms-2">مستقیم به انبار بدون نیاز به تایید</span>
                                                            <span className="font10 mt-1 text-danger">تنها درصورتی که امضا شما مورد تایید واحد انبار باشد میتوانید از این آیتم استفاده کنید!</span>
                                                        </InputGroup>
                                                    </Form.Group> : null}
                                            </Row>
                                        </Col>
                                    ...
                                    <Row className='mt-4 justify-content-center'>
                                        ...
                                        <Col md='4' xxl='2'>
                                            <Button variant='secondary' type='reset' className='w-100' onClick={() => { dispatch(RsetResetFormModal(true)) }}>
                                                انصراف
                                            </Button>
                                        </Col>
                                    </Row>
                        ...
                        {resetFormModal ? <ResetFormModal resetFunc={handleResetWarehouseReq} /> : null}
