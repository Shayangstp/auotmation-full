...
import {
  ...
  handleSelectedPurchaseItem,
  selectPurchaseSendItemsModal,
  selectPurchasedItemModal,
  RsetPurchasedItemModal,
  selectSelectedPurchaseItems,
    ...
    if (
      ...
      return (
        <input
          ...
          onChange={(e) => {
            dispatch(handleSelectedPurchaseItem({ e: e, request: request }));
          }}
        />
      );
    }
    ...
    if (requests.length !== 0) {
      for (var i = 0; i < requests.length; i++) {
        var tableItem = {
          ...
          reqAcceptedAmount:
            requests[i].acceptedAmount !== null
              ? xssFilters.inHTMLData(requests[i].acceptedAmount)
              : "",
          reqAcceptedPriority:
            requests[i].acceptedPriorityName !== null
              ? xssFilters.inHTMLData(requests[i].acceptedPriorityName)
              : "",
          reqBuyer:
            requests[i].buyerName !== null
              ? xssFilters.inHTMLData(requests[i].buyerName)
              : "",
      ...
      if (requests.length !== 0) {
        for (var i = 0; i < requests.length; i++) {
          var tableItem = {
            ...
            reqAmount: xssFilters.inHTMLData(requests[i].itemAmount),
            reqUnitName: xssFilters.inHTMLData(requests[i].itemUnitName),
            reqPriorityName: xssFilters.inHTMLData(
              requests[i].itemPriorityName
            ),
            reqStatus: xssFilters.inHTMLData(requests[i].statusName),
            reqAcceptedAmount:
              requests[i].acceptedAmount !== null
          ...
          <div className="mb-2">
            ...
            <Button
              ...
              onClick={() => {
                ...
                dispatch(
                  handleAllItems({ typeId: 9, filterValues: filterValues })
                );
              }}
            >
