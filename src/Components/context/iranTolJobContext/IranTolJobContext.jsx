...
const IranTolJobContext = ({ children }) => {
  ...
  const { reqFiles, setReqFiles } = addFileContext;
  ...
  const [iTJProjectTypeWithAllSelect, setITJProjectTypeWithAllSelect] =
    useState([{ value: "", label: "همه" }]);
  ...
  const [iTJProjectToolWithAllSelect, setITJProjectToolWithAllSelect] =
    useState([{ value: "", label: "همه" }]);
  ...
  const handleITJProjectNameValidation = () => {
    if (iTJProjectNameRef.current.value === "") {
      document
        .getElementById("iTJProjectName-required")
  ...
  const handleITJProjectTypeValidation = () => {
    if (iTJProjectTypeRef.current.props.value === "") {
      document
        .getElementById("iTJProjectType-required")
        .classList.remove("d-none");
      return false;
    } else {
      document
  ...
  const handleITJToolTypeValidation = () => {
    if (iTJProjectToolRef.current.props.value === "") {
      document
        .getElementById("iTJProjectTool-required")
        .classList.remove("d-none");
      return false;
    } else {
      document
  ...
  const handleITJProjectAmountValidation = () => {
    if (iTJProjectAmountRef.current.value === "") {
      document
        .getElementById("iTJProjectAmount-required")
        .classList.remove("d-none");
      return false;
    } else {
      document
  ...
  const handleITJProjectValidation = () => {
    ...
    if (
      ...
      projectToolValidation &&
      projectAmountValidation
    ) {
      return true;
    } else {
      return false;
  ...
  const handleConfirmITJReq = async () => {
    try {
      if (handleITJProjectValidation()) {
        ...
        const iTJReqValues = {
          ...
          deadline:
            iTJProjectDeadline !== null
              ? iTJProjectDeadline.format("YYYY/MM/DD")
              : null,
        ...
        console.log(iTJReqValues, files);
        const iTJReqRes = await postITJReq(iTJReqValues, files);
        console.log(iTJReqRes);
  ...
  const handleITJReqActionToPersons = async () => {
    try {
      ...
      if (toPersons !== "") {
        const actionToPersonsRes = await postActionToPersons(
          actionReqId,
          1,
          toPersons
        );
  ...
  const handleWorkDeps = async () => {
    try {
      ...
      if (
        workDepsRes.data.length !== undefined &&
        workDepsRes.data.length !== 0
      ) {
  ...
  const handleWorkTypes = async () => {
    try {
      ...
      if (
        workTypesRes.data.length !== undefined &&
  ...
  const handleWorkDevices = async () => {
    try {
      ...
      if (
        workDevicesRes.data.length !== undefined &&
        workDevicesRes.data.length !== 0
      ) {
  ...
  const handleWorkOperators = async () => {
    try {
      const workOperatorsRes = await getToPersonByRole(
        "65",
        user.Location,
        user.CompanyCode,
        1,
  ...
  const handleWorkFromDateValidation = () => {
    if (workFromDateRef.current.props.value === null) {
      document
        .getElementById("workFromDate-required")
        .classList.remove("d-none");
      return false;
    } else {
  ...
  const handleWorkPriorityValidation = () => {
    if (workPriorityRef.current.value === "") {
      document
        .getElementById("workPriority-required")
        .classList.remove("d-none");
      return false;
    } else {
  ...
  const handleWorkValidation = () => {
    ...
    if (
      workDepValidation &&
      workTypeValidation &&
      workFromDateValidation &&
      worktoDateValidation &&
      workPriorityValidation
    ) {
      return true;
    } else {
      return false;
  ...
  const [iTJProjectMaterialsAmount, setITJProjectMaterialsAmount] =
    useState("");
  ...
  const handleMaterialNameValidation = () => {
    if (iTJProjectMaterialsNameRef.current.value === "") {
      document
        .getElementById("iTJProjectMaterialsName-required")
        .classList.remove("d-none");
      return false;
    } else {
      document
  ...
  const handleMaterialUnitValidation = () => {
    if (iTJProjectMaterialsUnitRef.current.props.value === "") {
      document
        .getElementById("iTJProjectMaterialsUnit-required")
        .classList.remove("d-none");
      return false;
    } else {
      document
  ...
  const handleMaterialAmountValidation = () => {
    if (iTJProjectMaterialsAmountRef.current.value === "") {
      document
        .getElementById("iTJProjectMaterialsAmount-required")
        .classList.remove("d-none");
      return false;
    } else {
      document
  ...
  const handleMaterialValidation = () => {
    ...
    if (
      ...
      materialUnitValidation &&
      materialAmountValidation
    ) {
      return true;
    } else {
      return false;
    ...
    try {
      ...
      if (data.type === "accepted") {
        ...
        if (
          ...
          user.Roles.some((role) => role === "35")
        ) {
          // تاییدیه های مدیریت
          actionCode = 9;
          toPersonsRes = await getToPersonByRole(
            "14",
            user.Location,
            user.CompanyCode,
            1,
          ...
          if (reqFiles.length !== 0) {
            ...
            files = data;
            const fileUploadRes = await addITJReqFile(
              currentReqInfo.serial,
              currentReqInfo.requestId,
              files
            );
          ...
          if (workItems.length !== 0) {
            if (iTJProjectMaterials.length !== 0) {
              actionCode = 42;
              const anbartoPersonsRes = await getToPersonByRole(
                "3, 35",
                user.Location,
                user.CompanyCode,
                1,
              ...
              const materialList = iTJProjectMaterials.map((item) => {
                return {
                  ...
                  comment: item.desc,
                  place: {
                    main: "ایران تول - برنامه ریزی",
                    subPlace: currentReqInfo.reqInfo.serial_number,
                  },
                };
              });
              const workAndMaterialsRes = await patchWorkAndMaterials(
