...
import {
    checkPermission, gotPermissions, getCompanies, getCoDepartments, getAllUsersWithOffice,
    getAllUsers, getAllStatuses, getProvinces, getCities, getTodayDate, getDaysFood, getDepartmentUsers,
    getToPersonByRole, getCurrentReqInfo, getCurrentReqHistory, getNotSeenReqs, getCoUsers, checkReqUpdateDate,
    getCitiesByType, getProInfo, getRequestsList
...
import {
    postAction, getPurchaseList, updateDeadLinePriority,
    deletedReqItem, editWarehouseItem, updateRequest
} from "../../Services/warehouseReqService";

import { addTripOptions } from '../../Services/officeServices';

import { selectUser, handleLastNewReqs, selectLastNewReqs, selectUserNotFoundModal } from '../Slices/mainSlices';
import { useDispatch } from 'react-redux';

import UserInfoM from "../Modals/UserModals/UserInfoModal";
import { handleReqsList } from '../Slices/mainSlices';
import {
    RsetAcceptReqModal, selectNextAcceptReqModal, RsetNextAcceptReqModal, RsetCancelReqModal, RsetEditReqModal,
    RsetViewReqModal, RsetReqHistoryModal, selectAcceptReqComment, RsetAcceptReqComment, selectCancelReqComment, RsetCancelReqComment
} from '../Slices/modalsSlice';
import { selectCurrentReqInfo, selectCurrentReqItems } from "../Slices/currentReqSlice";
...
const MainContext = ({ children }) => {
    ...
    const handleCheckPermission = async (menuId) => {
        try {
            ...
            if (permissions === null || permissions === 'expired') {
                ...
                const check = () => {
                    const filteredItems = oldPermissions.filter(item => item === menuId);
                    return filteredItems;
                }
                const per = check();
                if (per.length !== 0) {
    ...
    const handleAllUsersWithOffice = async () => {
        try {
            if (user.CompanyCode !== null) {
                const { data } = await getAllUsersWithOffice(user.CompanyCode, user.Location);
                if (data.code === 412) {
    ...
    if (allUsersWithOffice.length !== 0) {
        allUsersWithOfficeSelect = allUsersWithOffice.map(user => {
            return { value: user._id, label: user.FirstName + ' ' + user.LastName + ' (' + user.CompanyName + ')' };
        })
    } else {
    ...
    if (allUsers.length !== 0) {
        allUsers.map(data => {
            usersSelect.push({ value: data._id, label: (data.first_name + " " + data.last_name) });
        })
    }
    ...
    const handleCitiesByType = async (type) => {
        try {
            ...
            if (citiesByTypeRes.data.length !== undefined && citiesByTypeRes.data.length !== 0) {
    ...
    const handleGetDaysFood = async () => {
        try {
            ...
            if (data.message === undefined && data.length !== 0 && Array.isArray(data)) {
                setFoods(data);
            } else {
                setFoods([]);
            }
    ...
    const handleCompanyUsers = async (info) => {
        try {
            const coUsersRes = await getCoUsers(info, user.CompanyCode, user.Location);
    ...
    const [reqItemReceivedDetailsModal, setReqItemReceivedDetailsModal] = useState(false);
    ...
    const handleGetCurrentReqComments = async (commentStatus, reqId, type) => {
        setLoading(true);
        try {
            const commentsRes = await getCurrentReqHistory(commentStatus, reqId, type);
    ...
    const handleOfficeLeaveAccept = async (actionType) => {
        ...
        if (data.type === "accepted") {
            ...
            if (currentReqInfo.typeId === 4) {
                var acceptReqValues;
                if (actionType === true) {
                    if (currentReqInfo.lastActionCode === 0 || currentReqInfo.lastActionCode === 20 || currentReqInfo.lastActionCode === 13) {
                        // go to edari
                        var toPersons = [];
                        var actionCode = 0;
                        if (currentReqInfo.leaveKindName === "ساعتي" && (currentReqInfo.lastActionCode === 0 || currentReqInfo.lastActionCode === 20)) {
                            toPersons = await getToPersonByRole('21', user.Location, user.CompanyCode, 1, null, '0');
                            actionCode = 13;
                        } else {
                            toPersons = await getToPersonByRole('19, 20', user.Location, user.CompanyCode, 1, null, '0');
                            actionCode = 21;
                        }
                ...
                if (actionType === 'sendToUpSup') {
                    ...
                    if (currentReqInfo.tripType.code === 1) {
                        // روشن دل
                        toPersons = await getToPersonByRole('44', user.Location, user.CompanyCode, 1, null, '0');
                    } else {
                        //لیاقت 
                        toPersons = await getToPersonByRole('45', user.Location, user.CompanyCode, 1, null, '0');
                    }
                    if (toPersons.data.code === 415) {
                        var toPersonsArr = [];
                        toPersons.data.list.map(async (person) => {
                    ...
                    if (companyCar === true) {
                        if (driver !== '') {
                            tripOptions = {
                                driver: driver.value,
                                startDate: officeMissionToDate !== null ? officeMissionToDate.format('YYYY/MM/DD') : null,
                            }
                            const addTripOptionsRes = await addTripOptions(currentReqInfo.requestId, tripOptions);
    ...
    const [purchaseItemsByWarehouseReq, setPurchaseItemsByWarehouseReq] = useState([]);
    const handleAcceptReq = async (acceptType) => {
        const { data } = await checkReqUpdateDate(currentReqInfo.requestId, currentReqInfo.lastActionId, 2);
        if (data.type === "accepted") {
                        ...
                        if (actionRes.data.code === 415) {
                            ...
                            currentReqItems.map(async item => {
                                const itemValues = {
                                    ...
                                    rattletrap: item.rattletrap !== null ? item.rattletrap.value : 0,
                                    consumable: item.consumable !== null ? item.consumable.value : 0,
                                    borrowed: item.borrowed !== null ? item.borrowed.value : 1,
                                    invKeeperComment: item.invKeeperComment !== null ? item.invKeeperComment : undefined,
                                    action: 3,
                                    userId: localStorage.getItem('id')
                                }
                                const patchItemValuesRes = await editWarehouseItem(item.itemId, itemValues);
    ...
    const handleAcceptPurchaseReq = async () => {
        if (currentReqInfo.lastActionCode === 0) {
            const toPersons = await getToPersonByRole('38', 1, user.CompanyCode, 1, null, 1);
            if (toPersons.data.code === 415) {
                var toPersonsArr = [];
                toPersons.data.list.map(async (person) => {
                    toPersonsArr.push(person.value)
    ...
    const [companyManagerIdFilterSelect, setCompanyManagerIdFilterSelect] = useState('');
    const [supportManagerIdFilterSelect, setSupportManagerIdFilterSelect] = useState('');
    const [purchasingOfficerIdFilterSelect, setPurchasingOfficerIdFilterSelect] = useState('');
    ...
    const [supportManagerUsersFilterSelect, setSupportManagerUsersFilterSelect] = useState([]);
    const [purchasingOfficerUsersFilterSelect, setPurchasingOfficerUsersFilterSelect] = useState([]);
